...
Выбранный сервис: **Moozza** плеер (альтернативный плеер для VK, так что владелец серверов - VK)
...
- **Куда это сохраняется:**  на сервере максимум обновляется дата последнего визита, а Moozza обновляет кэш, если появились свежие песни.
...
>так как нажать два раза поиск в приложении просто невозможно, тут будет действие **"добавление песни к себе".** И тут выбран случай, когда ,все же, Moozza спрашивает у сервера, а так песня просто молча добавится повторно.
...
Предположу, что возможность дубликатов вообще существует из-за того, что VK, наверное, хранит в базе данных порядковый номер песен в профиле, или еще какое-то иное значение (дату добавления?), тогда в таблицах нет конфликта из-за дубля...
